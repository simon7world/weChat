...
public abstract class Constants {
	...
	private static final String ACCESS_TOKEN_URL = CGI_BIN
			+ "token?grant_type=client_credential&appid=%s&secret=%s";
	private static final String JSAPI_TICKET_URL = CGI_BIN
	...
	private static final String ENTERPRISE_ACCESS_TOKEN_URL = ENTERPRISE_CGI_BIN
			+ "gettoken?corpid=%s&corpsecret=%s";
	private static final String ENTERPRISE_JSAPI_TICKET_URL = ENTERPRISE_CGI_BIN
			+ "get_jsapi_ticket?access_token=%s";

	public static String ACCESS_TOKEN_URL(final String id, final String secret) {

		return String.format(ENTERPRISE ? ENTERPRISE_ACCESS_TOKEN_URL
				: ACCESS_TOKEN_URL, id, secret);
	}

	public static String JSAPI_TICKET_URL(final String token) {

		return String.format(ENTERPRISE ? ENTERPRISE_JSAPI_TICKET_URL
				: JSAPI_TICKET_URL, token);
	}
